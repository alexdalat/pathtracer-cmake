...
Sphere::Sphere(glm::vec3 const& position, float radius, Material const& material) {
  this->v1 = position;
  this->radius = radius;
...
float Sphere::calculateIntersection(Ray const& ray) {
  ...
  if (t1 > 0.000001) return t1;
  if (t2 > 0.000001) return t2;
  return -1;
}
...
glm::vec3 Sphere::calculateNormal(glm::vec3 const& point, glm::vec3 const& dir) {
...
glm::vec3 Sphere::calculateUVCoordinates(glm::vec3 const& collisionPoint,
  ...
  return glm::vec3(atan2(normal.x, normal.z) / (2.0f * M_PI) + 0.5f,
                   normal.y * 0.5f + 0.5f, 0.0f); // TODO: optimize
}
